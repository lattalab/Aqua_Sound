...
static GstFlowReturn new_sample(GstAppSink *appsink, gpointer user_data) {
    ...
    while (audio_buffer.size() >= AUDIO_LEN) {
        ...
        std::string filename = dest_txt_folder_name + "/mel_spec_segment_" + std::to_string(segment_count++) + ".txt";
        ...
        if (msg != nullptr) {
            switch (GST_MESSAGE_TYPE(msg)) {
                ...
                case GST_MESSAGE_EOS: // Here, deal with the last segment, which is padding the last audio into `AUDIO_LEN`
		            ...
		            if ((segment_count == 1 && audio_buffer.size() > 0 && audio_buffer.size() < AUDIO_LEN) || (audio_buffer.size() >= 144000 && audio_buffer.size() < AUDIO_LEN)) {
