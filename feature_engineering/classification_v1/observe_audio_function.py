...
def load_audio(filepath, SAMPLE_RATE):  # load the audio
...
def plot_all(audio): 
    ...
    ampSP = np.abs(sp)
    # Calculate the frequency axis
    freq_axis = np.fft.fftfreq(len(sp), 1 / SAMPLE_RATE)
    # the magnitude spectrum
    plt.plot(freq_axis[:len(freq_axis)//2], ampSP[:len(freq_axis)//2])  # Only plot the positive frequencies
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('Magnitude')
    ...
    fig = librosa.display.specshow(spec, x_axis='time', y_axis='mel', sr=SAMPLE_RATE, hop_length=HOP_LEN, cmap='viridis')
    plt.title("Spectrogram",fontsize=17)

    plt.subplot(224)
    reduced_noise = denoise(audio)
    # spec = get_mel_spectrogram(audio) 
    spec = librosa.feature.melspectrogram(y=reduced_noise, sr=SAMPLE_RATE, fmax=FMAX, n_mels=N_MELS, hop_length=HOP_LEN, n_fft=N_FFT)
    spec = librosa.power_to_db(spec)  # Turn into log-scale
    # plot_mel_spectrogram(spec)
    fig = librosa.display.specshow(spec, x_axis='time', y_axis='mel', sr=SAMPLE_RATE, hop_length=HOP_LEN, cmap='viridis')
    plt.title("Spectrogram",fontsize=17)

    plt.title("Denoise_Spectrogram",fontsize=17)
